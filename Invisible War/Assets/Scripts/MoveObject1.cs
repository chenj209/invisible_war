...
public class MoveObject1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.O) && item != null)// pick up
        {
            PickUp(item);
        }
        if (Input.GetKeyDown(KeyCode.R))// get the start time of pressing the key
        {
            startTime = Time.time;
        }
        else if (Input.GetKeyUp(KeyCode.R))// throw or drop
        {
            Throw();
        }
    }
    ...
    private GameObject DetectItem()
    {
        ...
        pickUpText.text = @"Press O to pick up
Press R to throw";
